globalThis.__VINEXT_LAZY_CHUNKS__ = ["assets/page-D5LYlF9G.js","assets/layout-segment-context-C2yZHxO6.js"];
...
function getDraftSecret() {
	return "09608296-141a-4ac5-bf09-d7f93c0241ec";
}
/**
...
var NextURL = class NextURL {
	...
	*/
	get buildId() {
		return "de91cd0f-45a7-43da-ac38-2a8e213a432a";
	}
};
...
*/
function appIsrCacheKey(pathname, suffix, buildId = "de91cd0f-45a7-43da-ac38-2a8e213a432a") {
...
function createAppPageArtifactCompatibility(element, routePattern) {
	...
	return createArtifactCompatibilityEnvelope({
		...
		deploymentVersion: "de91cd0f-45a7-43da-ac38-2a8e213a432a",
		rootBoundaryId
	});
}
/**
...
var coupleSettings = sqliteTable("couple_settings", {
...
var statuses = sqliteTable("statuses", {
	role: text("role").primaryKey(),
	emoji: text("emoji").notNull(),
	label: text("label").notNull(),
	updatedAt: text("updated_at").notNull().default(sql`CURRENT_TIMESTAMP`)
});
var pokes = sqliteTable("pokes", {
...
//#endregion
//#region app/api/couple/route.ts
...
async function ensureSchema() {
	...
	await getDb().insert(coupleSettings).values({
		id: 1,
...
async function readAll() {
	...
	const [settingsRows, statusRows, messageRows, wishRows, pokeRows] = await Promise.all([
		...
		db.select({ count: sql`count(*)` }).from(pokes).where(sql`date(${pokes.createdAt}) = date('now')`)
	]);
	return {
...
async function GET(request) {
	if (!isAuthorized(request)) return Response.json({ error: "暗号不对，再想一想？" }, { status: 401 });
	try {
		await ensureSchema();
...
async function POST(request) {
	...
	try {
		...
		} else if (payload.type === "poke" && validRole(payload.role)) await db.insert(pokes).values({ author: payload.role });
		...
		} else return Response.json({ error: "这项操作还没有准备好" }, { status: 400 });
		return Response.json(await readAll());
	} catch {
...
var routes = [{
	...
	ids: {
		"route": "route:/api/couple",
		"page": null,
		"routeHandler": "route-handler:/api/couple",
		"rootBoundary": "root-boundary:/",
		"layouts": ["layout:/"],
		"templates": [],
		"slots": {}
